...
{
    public static class SkyQueueGS2
    {
        #region Fields
        ...
        private static string _portName2Reconnect;
        ...
        private static void StartUsbWatcher(string portName)
        {
            ...
            _portName2Reconnect = null;
            try
            {
                // DONE MRXXX added EventType = 2 to check USB re-connected events as well.  Verify this works as expected.
                var query = new WqlEventQuery("SELECT * FROM Win32_DeviceChangeEvent WHERE EventType = 3 OR EventType = 2");
        ...
        private static void OnUsbDeviceChanged(object sender, EventArrivedEventArgs e)
        {
            ...
            try
            {
                ...
                using (var results = searcher.Get())
                {
                    if (results.Count > 0)
                    {
                        // DONE: MRXXX: reopen serial port
                        ReconnectSerialPort(_portName2Reconnect);
                        if (Serial != null)
                            _portName2Reconnect = null;

                        // Port is still present — restore the name so future events can check again.
                        monitorItem.Message += System.Environment.NewLine + $"USB hardware — port {portName} AVAILABLE AGAIN";
                        MonitorLog.LogToMonitor(monitorItem);
                        Interlocked.CompareExchange(ref _monitoredPortName, portName, null);
                        return;
                    }
                }
                monitorItem.Message += System.Environment.NewLine + $"USB hardware — port {portName} DISCONNECTED";
                MonitorLog.LogToMonitor(monitorItem);
                // DONE MRXXXX Set _portName2Recconnect to detected port
                Interlocked.CompareExchange(ref _portName2Reconnect, portName, null);
                // Restore the name so future events can check again.
                Interlocked.CompareExchange(ref _monitoredPortName, portName, null);
                return;
            }
            catch (Exception ex)
            {
                monitorItem.Message += System.Environment.NewLine + $"USB hardware — port {portName} ERROR: {ex.Message}";
                MonitorLog.LogToMonitor(monitorItem);
            }
        }

        // DONE: MRXXX: Method ADDED to attempt to re-open the serial port when it is detected as disconnected and then reconnected.
        private static void ReconnectSerialPort(string portName)
        {
            if (string.IsNullOrEmpty(portName)) return;
            ...
            var monitorItem = new MonitorEntry
            {
                ...
                Method = MethodBase.GetCurrentMethod()?.Name,
                Thread = Thread.CurrentThread.ManagedThreadId,
                Message = $"USB serial port {portName} is RECONNECTING"
            };

            try
            {
                var sp = Serial as System.IO.Ports.SerialPort;
                var readTimeout = TimeSpan.FromMilliseconds(Serial?.ReadTimeout ?? 5000);

                var options = SerialOptions.DiscardNull
                    | (sp.DtrEnable ? SerialOptions.DtrEnable : SerialOptions.None)
                    | (sp.RtsEnable ? SerialOptions.RtsEnable : SerialOptions.None);
                var baudRate = sp.BaudRate;
                var handshake = sp.Handshake;
                var dataBits = sp.DataBits;
                ...
                if (Serial?.IsOpen == true)
                {
                    monitorItem.Message += System.Environment.NewLine + $"USB serial port {portName} is RECONNECTED";
                    MonitorLog.LogToMonitor(monitorItem);
                    return;
                }
                monitorItem.Message += System.Environment.NewLine + $"USB serial port {portName} UNEXPECTEDLY CLOSED";
                MonitorLog.LogToMonitor(monitorItem);
                return;
            ...
            {
                monitorItem.Message += System.Environment.NewLine + $"USB serial port {portName} re-connect ERROR: {ex.Message}";
                MonitorLog.LogToMonitor(monitorItem);
                Serial = null;
            }
