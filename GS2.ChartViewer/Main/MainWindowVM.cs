...
using GS2.ChartViewer.Controls.Dialogs;
using GS2.ChartViewer.Helpers;
...
namespace GS2.ChartViewer.Main
{
    internal class MainWindowVm : ObservableObject, IDisposable
    {
        ...
        private void ChartZoomCmd(object param)
        {
            ...
            catch (Exception ex)
            {
                ...
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Server, Category = MonitorCategory.Interface, Type = MonitorType.Error, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $" {ex.Message}" };
