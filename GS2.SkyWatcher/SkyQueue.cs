...
namespace GS2.SkyWatcher
{
    public static class SkyQueueGS2
    {
        ...
        private static void OnUsbDeviceChanged(object sender, EventArrivedEventArgs e)
        {
            ...
            try
            {
                ...
                using (var results = searcher.Get())
                {
                    if (results.Count > 0)
                    {
                        ...
                        monitorItem.Message += $"|USB hardware — port {portName} AVAILABLE AGAIN";
                        MonitorLog.LogToMonitor(monitorItem);
                        Interlocked.CompareExchange(ref _monitoredPortName, portName, null);
                        return;
                    }
                }
                monitorItem.Message += $"|USB hardware — port {portName} DISCONNECTED";
                MonitorLog.LogToMonitor(monitorItem);
            ...
            {
                monitorItem.Message += $"|USB hardware — port {portName} ERROR: {ex.Message}";
                MonitorLog.LogToMonitor(monitorItem);
            }
        }
        ...
        private static void ReconnectSerialPort(string portName)
        {
            ...
            try
            {
                ...
                if (Serial?.IsOpen == true)
                {
                    monitorItem.Message += $"|USB serial port {portName} is RECONNECTED";
                    MonitorLog.LogToMonitor(monitorItem);
                    return;
                }
                monitorItem.Message += $"|USB serial port {portName} UNEXPECTEDLY CLOSED";
                MonitorLog.LogToMonitor(monitorItem);
                return;
            ...
            {
                monitorItem.Message += $"|USB serial port {portName} re-connect ERROR: {ex.Message}";
                MonitorLog.LogToMonitor(monitorItem);
                Serial = null;
            }
