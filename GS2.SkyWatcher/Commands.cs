...
namespace GS2.SkyWatcher
{
    ...
    public class Commands
    {
      ...
      //  public DateTime LastJ2RunTime { get; private set; }
        ...
        /// </summary> 
        internal bool IsConnected => SkyQueueGS2.Serial.IsOpen && MountConnected;
        ...
        private void GetStepsPerRevolution(AxisId axis)
        {
            ...
            if (SupportAdvancedCommandSet && AllowAdvancedCommandSet)
            {
                ...
                switch (axis)
                {
                    case AxisId.Axis1:
                        ...
                        if (SkyQueueGS2.CustomMount360Steps[0] > 0) { gearRatio = SkyQueueGS2.CustomMount360Steps[0]; } //Setup for custom :a
                        _axisGearRatios[0] = gearRatio;
                        break;
                    case AxisId.Axis2:
                        _stepsPerRev[1] = gearRatio;
                        if (SkyQueueGS2.CustomMount360Steps[1] > 0) { gearRatio = SkyQueueGS2.CustomMount360Steps[1]; } //Setup for custom :a
            ...
            { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Information, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $":{msg}|{axis}|{response}|{ _stepsPerRev[(int)axis]}|Custom:{ SkyQueueGS2.CustomMount360Steps[(int)axis]}" };
            MonitorLog.LogToMonitor(monitorItem);
        }
        ...
        internal double GetPecPeriod(AxisId axis)
        {
            ...
            var ax = (int)axis;
            if (SkyQueueGS2.CustomRaWormSteps[ax] > 0) { pecPeriod = SkyQueueGS2.CustomRaWormSteps[ax]; } // Setup custom mount worm steps
            //_peSteps[ax] = pecPeriod;
            var ret = pecPeriod;

            var monitorItem = new MonitorEntry
            { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Information, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $":s|{axis}|{response}|{pecPeriod}|Custom:{SkyQueueGS2.CustomRaWormSteps[ax]}" };
            MonitorLog.LogToMonitor(monitorItem);
            ...
            monitorItem = new MonitorEntry
            { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Information, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $":X000E|{axis}|{response}|{pecPeriod}|Custom:{SkyQueueGS2.CustomRaWormSteps[ax]}" };
            MonitorLog.LogToMonitor(monitorItem);
                ...
                try
                {
                    ...
                    if (acquiredLock)
                    {
                        ...
                        try
                        {
                            ...
                            for (var c = 0; c <= 10; c++)
                            {
                                SkyQueueGS2.Serial.DiscardInBuffer();
                                SkyQueueGS2.Serial.DiscardOutBuffer();
                                // send the request
        ...
        private string SendRequest(AxisId axis, char command, string cmdDataStr)
        {
            ...
            SkyQueueGS2.Serial.Write(commandStr.ToString());
        ...
        private static string ReceiveResponse()
        {
            ...
            var readTimeout = SkyQueueGS2.Serial.ReadTimeout;
            while (sw.ElapsedMilliseconds < readTimeout)
            {
                var data = SkyQueueGS2.Serial.ReadExisting();
