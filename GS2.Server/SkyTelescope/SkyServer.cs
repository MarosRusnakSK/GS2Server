    ...
    public static class SkyServer
    {
        ...
        static SkyServer()
        {
            try
            {
                ...
                SkyQueueGS2.StaticPropertyChanged += PropertyChangedSkyQueueGS2;
        ...
        public static double ControllerVoltage
        {
            get
            {
                try
                {
                    var status = new SkyGetControllerVoltage(SkyQueueGS2.NewId, AxisId.Axis1);
                    return SkyQueueGS2.GetCommandResult(status).Result;
                }
                catch (Exception)
                {
                    return double.NaN;
            ...
            get
            {
                switch (SkySettings.Mount)
                {
                    ...
                    case MountType.SkyWatcher:
                        _mountRunning = SkyQueueGS2.IsRunning;
                        break;
                }
        ...
        private static int SkyGoTo(double[] target, bool trackingState, SlewType slewType, CancellationToken token)
        {
            ...
            while (stopwatch.Elapsed.TotalSeconds <= timer)
            {
                if (!axis1Stopped)
                {
                    ...
                    var statusX = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis1);
                    var x = SkyQueueGS2.GetCommandResult(statusX);
                ...
                if (!axis2Stopped)
                {
                    ...
                    var statusY = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis2);
                    var y = SkyQueueGS2.GetCommandResult(statusY);
        ...
        private static int SkyPrecisionGoto(double[] target, SlewType slewType, CancellationToken token)
        {
            ...
            while (true)
            {
                ...
                while (loopTimer.Elapsed.TotalMilliseconds < 3000)
                {
                    ...
                    if (!axis1Done)
                    {
                        var status1 = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis1);
                        axis1Done = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(status1).Result);
                    }
                    if (axis1Done) { break; }
                }
                ...
                while (loopTimer.Elapsed.TotalMilliseconds < 3000)
                {
                    ...
                    if (!axis2Done)
                    {
                        var status2 = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis2);
                        axis2Done = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(status2).Result);
                    }
                    if (axis2Done) { break; }
                }
                loopTimer.Stop();
        ...
        /// <summary>
        /// Creates tasks that are put in the SkyQueueGS2
        /// </summary>
        /// <param name="taskName"></param>
                ...
                case MountType.SkyWatcher:
                    switch (taskName)
                    {
                        ...
                        case MountTaskName.DecPulseToGoTo:
                            ...
                            break;
                        case MountTaskName.CanAdvancedCmdSupport:
                            var skyCanAdvanced = new SkyGetAdvancedCmdSupport(SkyQueueGS2.NewId);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(skyCanAdvanced).Result), out bool pAdvancedResult);
                            CanAdvancedCmdSupport = pAdvancedResult;
                            break;
                        case MountTaskName.CanPpec:
                            var skyMountCanPPec = new SkyCanPPec(SkyQueueGS2.NewId);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(skyMountCanPPec).Result), out bool pPecResult);
                            CanPPec = pPecResult;
                            break;
                        case MountTaskName.CanPolarLed:
                            var skyCanPolarLed = new SkyCanPolarLed(SkyQueueGS2.NewId);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(skyCanPolarLed).Result), out bool polarLedResult);
                            CanPolarLed = polarLedResult;
                            break;
                        case MountTaskName.CanHomeSensor:
                            var canHomeSky = new SkyCanHomeSensors(SkyQueueGS2.NewId);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(canHomeSky).Result), out bool homeSensorResult);
                            CanHomeSensor = homeSensorResult;
                            break;
                        case MountTaskName.Capabilities:
                            var skyCap = new SkyGetCapabilities(SkyQueueGS2.NewId);
                            Capabilities = (string)SkyQueueGS2.GetCommandResult(skyCap).Result;
                            break;
                        case MountTaskName.Encoders:
                        ...
                        case MountTaskName.FullCurrent:
                            ...
                            break;
                        case MountTaskName.GetFactorStep:
                            var skyFactor = new SkyGetFactorStepToRad(SkyQueueGS2.NewId);
                            FactorStep = (double[])SkyQueueGS2.GetCommandResult(skyFactor).Result;
                            break;
                        case MountTaskName.LoadDefaults:
                        ...
                        case MountTaskName.PecTraining:
                            ...
                            break;
                        case MountTaskName.Pec:
                            var ppeOcn = new SkySetPPec(SkyQueueGS2.NewId, AxisId.Axis1, SkySettings.PPecOn);
                            var pPecOnStr = (string)SkyQueueGS2.GetCommandResult(ppeOcn).Result;
                        ...
                        case MountTaskName.SetSt4Guiderate:
                            ...
                            break;
                        case MountTaskName.SetSouthernHemisphere:
                            _ = new SkySetSouthernHemisphere(SkyQueueGS2.NewId, SouthernHemisphere);
                            break;
                        case MountTaskName.SetSnapPort1:
                            var sp1 = new SkySetSnapPort(SkyQueueGS2.NewId, 1, SnapPort1);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(sp1).Result), out bool port1Result);
                            SnapPort1Result = port1Result;
                            break;
                        case MountTaskName.SetSnapPort2:
                            var sp2 = new SkySetSnapPort(SkyQueueGS2.NewId, 2, SnapPort2);
                            bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(sp2).Result), out bool port2Result);
                        ...
                        case MountTaskName.SyncAltAz:
                            ...
                            break;
                        case MountTaskName.GetAxisVersions:
                            var skyAxisVersions = new SkyGetAxisStringVersions(SkyQueueGS2.NewId);
                            // Not used atm
                            AxesVersion = (string[])SkyQueueGS2.GetCommandResult(skyAxisVersions).Result;
                            break;
                        case MountTaskName.GetAxisStrVersions:
                            var skyAxisStrVersions = new SkyGetAxisStringVersions(SkyQueueGS2.NewId);
                            // Not used atm
                            _ = (string)SkyQueueGS2.GetCommandResult(skyAxisStrVersions).Result;
                            break;
                        case MountTaskName.MountName:
                            var skyMountType = new SkyMountType(SkyQueueGS2.NewId);
                            MountName = (string)SkyQueueGS2.GetCommandResult(skyMountType).Result;
                            break;
                        case MountTaskName.MountVersion:
                            var skyMountVersion = new SkyMountVersion(SkyQueueGS2.NewId);
                            MountVersion = (string)SkyQueueGS2.GetCommandResult(skyMountVersion).Result;
                            break;
                        case MountTaskName.StepsPerRevolution:
                            var skyMountRevolutions = new SkyGetStepsPerRevolution(SkyQueueGS2.NewId);
                            StepsPerRevolution = (long[])SkyQueueGS2.GetCommandResult(skyMountRevolutions).Result;
                            break;
                        case MountTaskName.StepsWormPerRevolution:
                            var skyWormRevolutions1 = new SkyGetPecPeriod(SkyQueueGS2.NewId, AxisId.Axis1);
                            StepsWormPerRevolution[0] = (double)SkyQueueGS2.GetCommandResult(skyWormRevolutions1).Result;
                            var skyWormRevolutions2 = new SkyGetPecPeriod(SkyQueueGS2.NewId, AxisId.Axis2);
                            StepsWormPerRevolution[1] = (double)SkyQueueGS2.GetCommandResult(skyWormRevolutions2).Result;
                            break;
                        case MountTaskName.StepTimeFreq:
                            var skyStepTimeFreq = new SkyGetStepTimeFreq(SkyQueueGS2.NewId);
                            StepsTimeFreq = (long[])SkyQueueGS2.GetCommandResult(skyStepTimeFreq).Result;
                            break;
                        case MountTaskName.SetHomePositions:
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    while (stopwatch.Elapsed.TotalMilliseconds <= 5000)
                    {
                        ...
                        Thread.Sleep(100);
                        var statusX = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis1);
                        axis1Stopped = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(statusX).Result);

                        var statusy = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis2);
                        axis2Stopped = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(statusy).Result);
...
/// <returns></returns>
        ...
        private static double[] GetRawDegrees()
        {
            ...
            switch (SkySettings.Mount)
            {
                case MountType.Simulator:
                    ...
                    break;
                case MountType.SkyWatcher:
                    var skyPositions = new SkyGetPositionsInDegrees(SkyQueueGS2.NewId);
                    actualDegrees = (double[])SkyQueueGS2.GetCommandResult(skyPositions).Result;
        ...
        private static double[] GetRawSteps()
        {
            ...
            switch (SkySettings.Mount)
            {
                case MountType.Simulator:
                    ...
                    break;
                case MountType.SkyWatcher:
                    var skySteps = new SkyGetSteps(SkyQueueGS2.NewId);
                    steps = (double[])SkyQueueGS2.GetCommandResult(skySteps).Result;
                ...
                if (IsMountRunning || (_lastUpdateStepsTime.AddMilliseconds(100) < HiResDateTime.UtcNow))
                {
                    switch (SkySettings.Mount)
                    {
                        case MountType.Simulator:
                            ...
                            break;
                        case MountType.SkyWatcher:
                            _ = new SkyUpdateSteps(SkyQueueGS2.NewId);
                            break;
                        default:
                            throw new ArgumentOutOfRangeException();
                    }
        ...
        private static double? GetRawSteps(int axis)
        {
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    {
                        case 0:
                            var b = new SkyGetAxisPositionCounter(SkyQueueGS2.NewId, AxisId.Axis1);
                            return Convert.ToDouble(SkyQueueGS2.GetCommandResult(b).Result);
                        case 1:
                            var c = new SkyGetAxisPositionCounter(SkyQueueGS2.NewId, AxisId.Axis2);
                            return Convert.ToDouble(SkyQueueGS2.GetCommandResult(c).Result);
                        default:
                            return null;
                    }
                default:
        ...
        public static Tuple<double?, DateTime> GetRawStepsDt(int axis)
        {
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    {
                        case 0:
                            var b = new SkyGetAxisPositionDate(SkyQueueGS2.NewId, AxisId.Axis1);
                            return SkyQueueGS2.GetCommandResult(b).Result;
                        case 1:
                            var c = new SkyGetAxisPositionCounter(SkyQueueGS2.NewId, AxisId.Axis2);
                            return SkyQueueGS2.GetCommandResult(c).Result;
                        default:
                            return null;
                    }
                default:
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    if (Math.Abs(stepsNeededDec) > 0)
                    {
                        ...
                        {
                            var statusy = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis2);
                            var axis2Stopped = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(statusy).Result);
                    ...
                    if (Math.Abs(stepsNeededRa) > 0)
                    {
                        ...
                        {
                            var statusX = new SkyIsAxisFullStop(SkyQueueGS2.NewId, AxisId.Axis1);
                            var axis1Stopped = Convert.ToBoolean(SkyQueueGS2.GetCommandResult(statusX).Result);
        ...
        private static bool MountConnect()
        {
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    var init = new SkyGetMotorCardVersion(SkyQueueGS2.NewId, AxisId.Axis1);
                    _ = (string)SkyQueueGS2.GetCommandResult(init).Result;
        ...
        private static void MountStart()
        {
            ...
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    SkyQueueGS2.Start(SkySystem.Serial, custom360Steps, customWormSteps, LowVoltageEventSet);
                    if (!SkyQueueGS2.IsRunning)
                    {
            ...
            {
                monitorItem.Message += System.Environment.NewLine + $" - Stopping mount at START";
                MountStop();
            }
        }
        ...
        private static void MountStop()
        {
            ...
            // NONE: MRXXX: Stop tracking conditionally by TrackingAtDisconnect setting
            Tracking = false;
            CancelAllAsync();
            AxesStopValidate();
            ...
            if (!SkyQueueGS2.IsRunning) return;

            var skyStats = SkyQueueGS2.Statistics;
            if (skyStats != null)
            {
                var skyStatsItem = new MonitorEntry
                {
                    ...
                    Method = MethodBase.GetCurrentMethod()?.Name,
                    Thread = Thread.CurrentThread.ManagedThreadId,
                    Message = $"SkyQueueGS2 Session Statistics: {skyStats}"
                };
        ...
        private static void PropertyChangedSkyQueueGS2(object sender, PropertyChangedEventArgs e)
        {
            switch (e.PropertyName)
            {
                case "IsPulseGuidingRa":
                    IsPulseGuidingRa = SkyQueueGS2.IsPulseGuidingRa;
                    break;
                case "IsPulseGuidingDec":
                    IsPulseGuidingDec = SkyQueueGS2.IsPulseGuidingDec;
                    break;
                case "Steps":
                    Steps = SkyQueueGS2.Steps;
        ...
        private static void CheckPecTraining()
        {
            switch (SkySettings.Mount)
            {
                ...
                case MountType.SkyWatcher:
                    ...
                    var pPecTrain = new SkyIsPPecInTrainingOn(SkyQueueGS2.NewId);
                    if (bool.TryParse(Convert.ToString(SkyQueueGS2.GetCommandResult(pPecTrain).Result), out bool bTrain))
                    {
                        PecTraining = bTrain;
