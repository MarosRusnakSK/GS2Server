...
namespace GS2.SkyApi
{
    ...
    [Guid("8C144982-B0D8-4E66-B892-92E65976C94D")]
    [ProgId("GS2.SkyApi")]
    ...
    public class Sky : ObjectBase, ISky
    {
        ...
        public bool AscomOn
        {
            ...
            set
            {
                ...
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{value}" };
        ...
        public void AxisGoToTarget(int axis, double targetPosition)
        {
            var monitorItem = new MonitorEntry
            {
                Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope,
            ...
            var command = new SkyAxisGoToTarget(SkyQueueGS2.NewId, validAxis, targetPosition);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{steps}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyAxisMoveSteps(SkyQueueGS2.NewId, validAxis, steps);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{guideRate}|{duration}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyAxisPulse(SkyQueueGS2.NewId, validAxis, guideRate, duration, backlashSteps, new CancellationToken());
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{rate}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyAxisSlew(SkyQueueGS2.NewId, validAxis, rate);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyAxisStop(SkyQueueGS2.NewId, validAxis);
            GetResult(command);
        }
        ...
        public void AxisStopInstant(int axis)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyAxisStopInstant(SkyQueueGS2.NewId, validAxis);
            GetResult(command);
        }
        ...
        public bool CanAxisSlewsIndependent
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanAxisSlewsIndependent(SkyQueueGS2.NewId);
        ...
        public bool CanAzEq
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanAzEq(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanDualEncoders
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanDualEncoders(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanHalfTrack
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanHalfTrack(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanHomeSensors
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanHomeSensors(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanPolarLed
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanPolarLed(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanPPec
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanPPec(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool CanWifi
        {
            ...
            {
                ValidateMount();
                var command = new SkyCanWifi(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public string CmdToMount(int axis, string cmd, string cmdData, string ignoreWarnings)
        {
            ...
            var command = new SkyCmdToMount(SkyQueueGS2.NewId, axis, cmd, cmdData, ignoreWarnings);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetAngleToStep(int axis, double angleInRad)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetAngleToStep(SkyQueueGS2.NewId, validAxis, angleInRad);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetAxisVersion(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetAxisVersions(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public string GetAxisStringVersion(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetAxisStringVersions(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public double GetAxisPosition(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetAxisPosition(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetAxisPositionCounter(int axis, bool raw)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetAxisPositionCounter(SkyQueueGS2.NewId, validAxis, raw);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetControllerVoltage(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetControllerVoltage(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetEncoderCount(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetEncoderCount(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double Get_j(int axis, bool raw)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetJ(SkyQueueGS2.NewId, validAxis, raw);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetFactorRadRateToInt(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetFactorRadRateToInt(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public long GetHighSpeedRatio(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetHighSpeedRatio(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public long GetHomePosition(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetHomePosition(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetLastGoToTarget(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetLastGoToTarget(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetLastSlewSpeed(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetLastSlewSpeed(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetLowSpeedGotoMargin(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetLowSpeedGotoMargin(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public string GetMotorCardVersion(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetMotorCardVersion(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetPecPeriod(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetPecPeriod(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        {
            ValidateMount();
            var command = new SkyGetPositionsAndTime(SkyQueueGS2.NewId, raw);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetRampDownRange(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetRampDownRange(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetSiderealRate(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetSiderealRate(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public double GetStepToAngle(int axis, long steps)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetStepToAngle(SkyQueueGS2.NewId, validAxis, steps);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public long GetStepsPerRevolution(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetStepsPerRevolution(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        public long GetStepTimeFreq(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyGetStepTimeFreq(SkyQueueGS2.NewId);
            var results = GetResult(command);
            switch (validAxis)
            {
                case AxisId.Axis1:
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = "Started" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var command = new SkyInitializeAxes(SkyQueueGS2.NewId);
            GetResult(command);
        }
        ...
        public bool IsConnected
        {
            get
            {
                var command = new SkyIsConnected(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool IsPPecInTrainingOn
        {
            ...
            {
                ValidateMount();
                var command = new SkyIsPPecInTrainingOn(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool IsPPecOn
        {
            ...
            {
                ValidateMount();
                var command = new SkyIsPPecOn(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool IsFullStop(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyIsAxisFullStop(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public bool IsHighSpeed(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyIsHighSpeed(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }

        /// <inheritdoc />
        public bool IsServerSkyWatcher => SkySettings.Mount == GS2.Server.SkyTelescope.MountType.SkyWatcher;
        ...
        public bool IsSlewing(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyIsSlewing(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public bool IsSlewingForward(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyIsSlewingForward(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public bool IsSlewingTo(int axis)
        {
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyIsSlewingTo(SkyQueueGS2.NewId, validAxis);
            var results = GetResult(command);
            return results.Result;
        }
        ...
        public string MountType
        {
            ...
            {
                ValidateMount();
                var command = new SkyMountType(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        public bool MountVersion
        {
            ...
            {
                ValidateMount();
                var command = new SkyMountVersion(SkyQueueGS2.NewId);
                var results = GetResult(command);
                return results.Result;
            }
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{on}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var command = new SkySetAlternatingPPec(SkyQueueGS2.NewId, on);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{position}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetAxisPosition(SkyQueueGS2.NewId, validAxis, position);
            GetResult(command);
        }
        ...
        public void SetAxisPositionCounter(int axis, int position)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetAxisPositionCounter(SkyQueueGS2.NewId, validAxis, position);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{stepsCount}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetBreakPointIncrement(SkyQueueGS2.NewId, validAxis, stepsCount);
            GetResult(command);
        }
        ...
        public void SetDecPulseToGoTo(bool on)
        {
            ...
            ValidateMount();
            var command = new SkySetDecPulseToGoTo(SkyQueueGS2.NewId, on);
            GetResult(command);
        }
        ...
        public void SetEncoder(int axis, bool on)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetEncoder(SkyQueueGS2.NewId, validAxis, on);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{on}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetFullCurrent(SkyQueueGS2.NewId, validAxis, on);
            GetResult(command);
        }
        ...
        public void SetGotoTargetIncrement(int axis, long stepsCount)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetGotoTargetIncrement(SkyQueueGS2.NewId, validAxis, stepsCount);
            GetResult(command);
        }
        ...
        public void SetHomePositionIndex(int axis)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetHomePositionIndex(SkyQueueGS2.NewId, validAxis);
            GetResult(command);
        }
        ...
        public void StartMotion(int axis)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkyStartMotion(SkyQueueGS2.NewId, validAxis);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{func}|{direction}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetMotionMode(SkyQueueGS2.NewId, validAxis, func, direction);
            GetResult(command);
        }
        ...
        public void SetPPec(int axis, bool on)
        {
            ...
            SkyServer.PPecOn = on;
            //var validAxis = ValidateAxis(axis);
            //var command = new SkySetPPec(SkyQueueGS2.NewId, validAxis, on);
            //GetResult(command);
        ...
        public void SetPPecTrain(int axis, bool on)
        {
            ...
            //var validAxis = ValidateAxis(axis);
            //var command = new SkySetPPecTrain(SkyQueueGS2.NewId, validAxis, on);
            //GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
            { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}|{stepSpeed}" };
            MonitorLog.LogToMonitor(monitorItem);

            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetStepSpeed(SkyQueueGS2.NewId, validAxis, stepSpeed);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{rate}" };
            MonitorLog.LogToMonitor(monitorItem);
        ...
        public void SetTargetPosition(int axis, double position)
        {
            ...
            ValidateMount();
            var validAxis = ValidateAxis(axis);
            var command = new SkySetTargetPosition(SkyQueueGS2.NewId, validAxis, position);
            GetResult(command);
        }
        ...
        {
            var monitorItem = new MonitorEntry
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Data, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = "API Shutdown" };
            MonitorLog.LogToMonitor(monitorItem);
        ...
        private static AxisId ValidateAxis(int axis)
        {
            switch (axis)
            {
                ...
                default:
                    ...
                    var monitorItem = new MonitorEntry
                    { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Error, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{axis}" };
                    MonitorLog.LogToMonitor(monitorItem);
        ...
        private static ISkyCommand GetResult(ISkyCommand command)
        {
            ...
            var result = SkyQueueGS2.GetCommandResult(command);
            ...
            if (result == null)
            {
                ...
                var monitorItem = new MonitorEntry
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Error, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{exception}" };
                MonitorLog.LogToMonitor(monitorItem);
            ...
            else
            {
                ...
                var monitorItem = new MonitorEntry
                { Datetime = GS2.Principles.HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Driver, Type = MonitorType.Error, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Thread.CurrentThread.ManagedThreadId, Message = $"{result.Exception}" };
                MonitorLog.LogToMonitor(monitorItem);
